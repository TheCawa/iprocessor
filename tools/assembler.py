...
OPCODES = {
    ...
    'WRINT':    {'op': 0x25, 'sf': 0x00, 'type': 'A3', 'len': 6},
    ...
    def second_pass(self, lines: List[str]) -> List[int]:
        ...
        for line_num, line in enumerate(lines, 1):
                    ...
                    if end_quote > 0:
                        ...
                        if rest.startswith(','):
                            for byte_str in rest[1:].split(','):
                                ...
                                if byte_str:
                                    code.append(self.parse_operand(byte_str))
                                    address += 1
                else:
            ...
            try:
                ...
                elif instr['type'] == 'J0':  # JMP addr24
                    imm = self.parse_operand(parts[1])
                    code.append(0x00)  # COND (для JMP.UNC)
                    code.append((imm >> 16) & 0xFF)
                    code.append((imm >> 8) & 0xFF)
                    code.append(imm & 0xFF)
                    address += 4
